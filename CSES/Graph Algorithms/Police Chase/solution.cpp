...
void bfs(int source, set<int>& s){
	...
	while (!q.empty()){
		...
		q.pop();
		for(int i : adj[curr]) {
			if(edges[curr][i].se < edges[curr][i].fi){
				if (s.find(i) == s.end()) s.insert(i), q.push(i);
			}
		}
	}
}
...
void solve(){
	cin >> n >> m;
	forn(i, 0, n){
		edges.pb(vector<pair<int, int>>(n, mp(0, 0)));
		tracker.pb(vector<bool>(n));
		adj.pb(vector<int>());
	}
	...
	reset();
	while(dfs(0, n-1, INT32_MAX)) reset();
	set<int> s;
	bfs(0, s);
	int ans = 0;
	for(int i : s) {
	...
	for(int i : s) {
		for(int j : adj[i]){
			if (s.find(j) == s.end()) cout << i + 1 << ' ' << j + 1 << endl;
		}
	}
}
