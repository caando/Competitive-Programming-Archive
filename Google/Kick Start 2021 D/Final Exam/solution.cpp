...
using vi = vector<int>;#include<bits/stdc++.h>
using namespace std;
using ll = long long;
using pii = pair<int, int>;
using pll = pair<ll, ll>;
...
void solve(){
    ...
    forn(i, 0, m){
        int s;
        cin >> s;
        auto i1 = p.lower_bound(mp(s, 0)), i2=p.lower_bound(mp(s, 0));
        if (i2 != p.begin()) i2--;
        pair<ll, ll> a = (*i1), b = (*i2);
        if (a.fi <= s && a.se >= s){
            cout << s << ' ';
            p.erase(a);
            if (a.fi <= s-1) p.insert(mp(a.fi, s-1));
            if (a.se >= s+1) p.insert(mp(s+1, a.se));
        } else if (b.fi <= s && b.se >= s){
            cout << s << ' ';
            p.erase(b);
            if (b.fi <= s-1) p.insert(mp(b.fi, s-1));
            if (b.se >= s+1) p.insert(mp(s+1, b.se));
        } else {
            if (abs(a.fi - s) < abs(b.se - s)){
                cout << a.fi << ' ';
                p.erase(a);
                if (a.se >= a.fi+1) p.insert(mp(a.fi+1, a.se));
            } else {
                cout << b.se << ' ';
                p.erase(b);
                if (b.fi <= b.se-1) p.insert(mp(b.fi, b.se-1));
            }
        }
    }
    cout << endl;
