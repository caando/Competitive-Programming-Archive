...
void solve(){
	...
	while(r-l > 2){
		...
		ll f1 = 0, f2 = 0;
		forn(i, 0, n){
			if (arr[i] > m1) f1 += b*(arr[i]-m1);
			if (arr[i] < m1) f1 += a*(m1-arr[i]);
		}
		forn(i, 0, n){
			if (arr[i] > m2) f2 += b*(arr[i]-m2);
			if (arr[i] < m2) f2 += a*(m2-arr[i]);
		}
        if (f1 > f2) l = m1;
        else r = m2;
		if (f1 < ans) ans = f1;
	...
	int m = (r+l)/2;
	forn(i, 0, n){
		if (arr[i] > m) f += b*(arr[i]-m);
		if (arr[i] < m) f += a*(m-arr[i]);
	}
	if (f < ans) ans = f;
	cout << ans;
}
