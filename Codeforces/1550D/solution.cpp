...
void solve(){
    ...
    cin >> n >> l >> r;
    ll ans = 0;
    if (n&1) ans += min(1 - l, r - n) * 2 * ((f[n] * invf[n/2] % mod) * invf[n-n/2] % mod) % mod;
    else ans += min(1 - l, r - n) * ((f[n] * invf[n/2] % mod) * invf[n-n/2] % mod) % mod;
    ans %= mod;
    forn(a, 1, n){
        int b = max(0, a - abs(1 - l - r + n));
        if (n&1) {
            if(n/2-a+1 >= 0 && n/2-b >= 0) ans += (f[n-a-b] * invf[n/2-a+1] % mod) * invf[n/2-b] % mod;
            if(n/2-a >= 0 && n/2-b+1 >= 0) ans += (f[n-a-b] * invf[n/2-a] % mod) * invf[n/2-b+1] % mod;
        } else {
            if(n/2-a >= 0 && n/2-b >= 0) ans += (f[n-a-b] * invf[n/2-a] % mod) * invf[n/2-b] % mod;
        }
        ans %= mod;
    }
    cout << ans << endl;
...
int main(){
    f[0] = 1, invf[0] = 1;
    forn(i, 1, 200001) f[i] = f[i-1] * i % mod, invf[i] = invf[i-1] * inv_mod(i, mod) % mod;
