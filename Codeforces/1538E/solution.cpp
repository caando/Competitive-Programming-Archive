	...
	forn(i, 0, n){
		...
		if (op == ":="){
			...
			forn(i, 0, s.size() - 3){
				bool ok = true;
				forn(j, 0, 4){
					if (haha[j] != s[i+j]) ok = false;
				}
				if (ok) count++;
			}
			m[a].fi = count;
			if (m[a].se == "haha") m[a].fi = 1;
		} else {
			string b, c, bin;
			cin >> b >> bin >> c;
			m[a].fi = m[b].fi + m[c].fi;
			string temp = m[b].se.substr(max(0, (int)m[b].se.size()-3), m[b].se.size()) + m[c].se.substr(0, min(3, (int)m[c].se.size()));
			int count = 0;
			forn(i, 0, temp.size() - 3){
				bool ok = true;
				forn(j, 0, 4){
					if (haha[j] != temp[i+j]) ok = false;
				}
				if (ok) count++;
			}
			m[a].fi += count;
			m[a].se = m[b].se.substr(0, min(3, (int)m[b].se.size())) + m[c].se.substr(max(0, (int)m[c].se.size()-3), m[c].se.size());
		}
	}
	cout << m[a].fi << endl;
}
