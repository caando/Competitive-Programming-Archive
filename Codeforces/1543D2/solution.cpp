...
void solve(){
    ...
    int p[n], dp[18];
    p[0] = 1;
    forn(i, 1, n) p[i] = p[i-1]*(k-1) % k;
    memset(dp, 0, sizeof dp);
    forn(i, 0, n){
        int curr = i, ans=0, d[18], res, mul=1;
        forn(j, 0, 18) d[j] = ((curr % k) * p[i] + dp[j]) % k, curr /= k;
        forn(j, 0, 18) ans += d[j] * mul, mul *= k;
        cout << ans << endl;
        cout.flush();
        cin >> res;
        if (res != 0) break;
        forn(j, 0, 18) dp[j] = (dp[j] * (k-1) + ans % k)% k, ans /= k;
    }
}
