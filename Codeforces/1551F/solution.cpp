...
void solve(){
    ...
    if (k == 2){
        ...
        ll ans = 0;
        forn(i, 0, n){
            vector<vector<int>> v = vector<vector<int>>(adjlist[i].size(), vector<int>(n, 0));
            ...
            forn(j, 0, n){
                ...
                forn(l, 0, adjlist[i].size()){
                    for(int m = k-1; m >= 1; m--) dp[m] += dp[m-1] * v[l][j], dp[m] %= mod;
                    dp[0] += v[l][j]; 
                }
                ans += dp[k-1];
            }
